...
export const callSoapService = async (url, methodName, parameterDetails) => {
  try {
    if (GlobalVariables.doConnectionParameter && typeof GlobalVariables.doConnectionParameter === 'object') {
      await callSoapServiceforMethods(url, "doConnection", GlobalVariables.doConnectionParameter);
    } else {
      console.warn('⚠️ doConnectionParameter is missing or invalid.');
    }

    const result = await callSoapServiceforMethods(url, methodName, parameterDetails);
    return result;
